...
이 프로젝트는 **분산된 IoT 데이터를 통합하고, 실시간 모니터링, 원격 제어, AI 기반 분석을 통해 지능형 IoT 관리 솔루션을 제공**하는 시스템의 설계 및 제안 자료를 웹 애플리케이션으로 구현한 것입니다.

### 문서의 목적

이 문서는 프로젝트의 기술적 세부사항, 아키텍처, 컴포넌트 구조를 상세히 분석하여 개발팀과 이해관계자가 프로젝트를 이해하고 구현하는 데 필요한 정보를 제공합니다.
...
이 프로젝트는 **사용자 그룹별로 최적화된 3가지 모드**를 제공하며, 각 모드는 **점진적으로 구체화되는 관계**를 가집니다:
...
**구성**: 7개 슬라이드 (제목 + 6개 콘텐츠 슬라이드)
- 각 슬라이드는 비즈니스 관점에서 핵심 내용을 요약하여 제시
- 기술적 세부사항보다는 비즈니스 가치와 효과에 중점

**대상**: 고객사, 경영진, 의사결정자

**다음 단계**: 
- → 기술 상세 모드에서 각 슬라이드의 기술적 상세 분석 제공
- → 설계 문서 모드에서 각 슬라이드의 구체적인 프로세스와 다이어그램 제공

#### 2. 기술 상세 모드 (Technical Detail Mode)

**명칭 변경 제안**: "대시보드 모드" → "기술 상세 모드"

**목적**: 상세 분석 및 기술 검토용 - **프레젠테이션 모드의 기술적 상세 버전**

**구성**: 7개 프레임으로 구성된 상세 분석
- 프레젠테이션 슬라이드의 각 내용을 기술적 관점에서 상세히 분석
- 각 프레임은 대응하는 프레젠테이션 슬라이드의 기술적 확장

**대상**: 기술팀, 아키텍트, 개발팀

**다음 단계**:
- → 설계 문서 모드에서 각 프레임의 내용을 더욱 상세하게 기술 문서로 설명

#### 3. 설계 문서 모드 (Design Document Mode) - 구체화 버전

**핵심 개념**: 설계 문서 모드는 **프레젠테이션 모드와 기술 상세 모드의 구체화된 상세 버전**입니다.

**목적**: 설계 문서 조회 및 검토용 - **최종 구체화 버전**

**구성**: 마크다운 문서 뷰어 및 Mermaid 다이어그램 렌더링
- **프레젠테이션 모드의 구체화**: 각 슬라이드의 내용을 기술적으로 상세히 설명
- **기술 상세 모드의 구체화**: 각 프레임의 내용을 기술 문서로 상세화

**주요 문서**:
- `PROCESS_FLOW.md`: 전체 시스템 프로세스를 Mermaid 다이어그램과 상세 설명으로 정리
...
- **URL 해시 기반 라우팅**: 각 모드와 섹션은 고유한 URL 해시를 가짐
  - 프레젠테이션: `#presentation/{slide-id}`
  - 대시보드: `#dashboard/{frame-id}`
  - 설계 문서: `#docs/{doc-id}/{section-id}`
- **UI 버튼**: 화면 우측 상단의 모드 전환 버튼으로 쉽게 전환 가능
- **브라우저 히스토리**: 뒤로/앞으로 버튼으로 네비게이션 지원

### 아키텍처 특징

- **단일 페이지 애플리케이션 (SPA)**: Vite + React 기반
